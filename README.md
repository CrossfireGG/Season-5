# Crossfire GG

**Online Tier-2 CS2 Tournament** | June 1 – July 10, 2026
**VRS Certified Event** — Structured in alignment with Valve Regional Standings (VRS) integrity guidelines

---

## Quick Links

- **Registration:** [Register Your Team](https://forms.gle/yThCP1swbPepPYZx7)
- **Payment (Due immediately after registration):** [PayPal Checkout](https://www.paypal.com/ncp/payment/6WWNTPRD68K5W)
- **Season 5 Registration Tracker:** [Live Signup Tracker](https://docs.google.com/spreadsheets/d/1ImKbO7W0yieecN4KKrEH4hHQELOZA7uLP4ka-JgLhtg/edit?usp=sharing)
- **Discord:** [discord.gg/PczPADKPvf](https://discord.gg/PczPADKPvf)
- **Twitter (X):** [@PlayCrossFireGG](https://x.com/PlayCrossFireGG)
- **Twitch:** [PlayCrossFire](https://www.twitch.tv/PlayCrossFire)
- **Twitch (Secondary):** [playcrossfirecs2](https://www.twitch.tv/playcrossfirecs2)
- **YouTube:** [@CrossfireGG](https://www.youtube.com/@CrossfireGG)
- **FACEIT Hub:** [CrossfireGG](https://www.faceit.com/en/organizers/1b5c9ed3-5233-4c58-8761-78f6ef5a0fd9/CrossfireGG)

---

## Tournament Classification

CrossfireGG operates as an open-registration Tier-2 structured competition. **Season 5 is a VRS Certified event.** Valve Regional Standings (VRS) are referenced for seeding integrity and competitive structure. This event does NOT utilize Direct VRS Invites. This is **not a casual or community cup** — it is a professionally-operated tournament adhering to competitive Counter-Strike 2 standards.

### Key Information

- **Season 5 Dates:** June 1 – July 10, 2026
- **Game:** Counter-Strike 2 (CS2)
- **Tournament Type:** Online Tier-2, VRS Certified, structured in alignment with Valve Regional Standings (VRS) integrity guidelines
- **Region:** Americas (North America)
- **Location:** Online
- **Teams:** 32 Teams
- **Entry Fee:** $150 per team
- **Payment:** Due immediately after registration submission via [PayPal](https://www.paypal.com/ncp/payment/6WWNTPRD68K5W)
- **Prize Pool:** $5,000 USD (1st: $3000 USD | 2nd: $1500 USD | 3rd: $500 USD)

---

## Roster & Core Policy

CrossfireGG does not enforce a full roster lock during the regular season or group stage. Teams may adjust players prior to playoffs, subject to tournament administration approval.

All teams must maintain a consistent Team Core (3 players) for competitive integrity.

### VRS Teams

- Must maintain at least three (3) players from the official VRS snapshot roster (May 2, 2026) at all times
- Changing one or two players does not affect VRS eligibility
- Falling below three snapshot players results in immediate disqualification to preserve VRS integrity

### Non-VRS Teams

- Must also maintain a consistent Core-3 identity for competitive integrity
- Administrative review may occur if the core identity changes

### Duplicate Core Restrictions

- **Duplicate core rosters are not allowed** — no two teams may share 3 or more core players
- Teams found in violation will be disqualified
- This rule ensures roster integrity across the competitive landscape

### Coach Substitution Rule (NEW for Season 5)

- Registered team coaches are permitted to substitute into matches as players, provided they are listed on the official roster prior to match start and meet all eligibility requirements.

---

## Seeding Integrity Policy

- Seeding reflects the core roster identity
- Core-3 continuity ensures fair seeding stability
- Seeding does not imply a full roster lock prior to playoffs
- Teams disqualified due to core violations will not retain seeded positions

---

## Tournament Format

### Group Stage

- **Format:** Round-Robin Group Stage
- **Structure:** 8 Groups × 4 Teams (32 teams total)
- **Match Type:** All matches are Best-of-3 (BO3)
- **Duration:** June 1 – June 18, 2026
- **Advancement:** Top 2 teams from each group advance to Playoffs (16 teams total)
- **Default Match Day:** Day 3 of each round window @ 8:00 PM EST
- Teams may reschedule matches within the group-stage window with mutual agreement

**Match Rounds:**

- Round 1: June 1 – June 4
- Round 2: June 5 – June 8
- Round 3: June 9 – June 12
- **Buffer / Make-up Matches:** June 13 – June 18 *(NEW for Season 5)*

**Break Week:** June 19 – June 22 *(shorter than Season 4)*

### Seeding Method

Teams are seeded using a **separated seeding methodology** to ensure competitive balance:

1. **Seeds 1-8** are placed in separate groups — one top seed per group
2. **Seeds 9-16** are distributed across all groups
3. **Seeds 17-24** are distributed across all groups
4. **Seeds 25-32** are distributed across all groups

This ensures each group contains a mix of skill levels and prevents stacking.

### Playoff Stage

- **Format:** 16-Team Single Elimination Bracket
- **Match Type:** All matches are Best-of-3 (BO3)
- **Duration:** June 23 – July 10, 2026
- **Default Match Day:** Day 3 of each round window @ 8:00 PM EST
- Reschedules permitted within each round window with admin approval

**Playoff Rounds:**

- Round 4 (Round of 16): June 23 – June 26
- Round 5 (Quarterfinals): June 27 – June 30
- Round 6 (Semi-Finals): July 1 – July 5
- Round 7 (Grand Finals): July 6 – July 10

---

## Registration & Eligibility Policy

### Open Registration

- **All teams enter via open registration** — Any eligible North American team may register until all 32 slots are filled
- **No Direct VRS Invites** — VRS standings are used only as a transparent reference for competitive seeding within an open registration format
- All current VRS-ranked teams are confirmed eligible to register
- **Register Here:** [https://forms.gle/yThCP1swbPepPYZx7](https://forms.gle/yThCP1swbPepPYZx7)
- **Payments Due Right After Submission:** [https://www.paypal.com/ncp/payment/6WWNTPRD68K5W](https://www.paypal.com/ncp/payment/6WWNTPRD68K5W)

### VRS Seeding Priority

Teams are seeded based on their standing in the **May 2, 2026 VRS Leaderboard**:

1. **VRS-ranked teams** receive seeding priority based on their most recent VRS rank (seeding priority does NOT mean invitation status)
2. **Non-VRS teams** are seeded by ESEA League Division and Win/Loss record
3. Teams appearing on both the Season 5 roster AND the VRS leaderboard are VRS-seeded
- **Final Seeding:** Publicly announced before Round 1 begins

### Region Eligibility

- **Region:** Americas (North America)
- All players must reside in North America
- All players must be able to connect to North American servers with stable connection
- No active VAC or platform bans permitted

---

## Tournament Timeline

| Date | Milestone |
|------|-----------|
| **May 2, 2026** | VRS Snapshot Taken |
| **Pre-June 1, 2026** | Group Assignments Revealed |
| **June 1, 2026** | Group Stage Begins (Round 1) |
| **June 12, 2026** | Group Stage Round 3 Ends |
| **June 13–18, 2026** | Buffer / Make-up Matches |
| **June 19–22, 2026** | Break Week |
| **June 23, 2026** | Playoff Stage Begins (Round of 16) |
| **July 10, 2026** | Grand Finals & Tournament Conclusion |

---

## Match Integrity & Administration

### Competitive Platform

- **Platform:** All matches hosted on FACEIT
- **Server Region:** North America (United States)
- **Server Settings:** Official competitive CS2 settings (MR12/3)

### Anti-Cheat Enforcement

- **Required:** FACEIT Anti-Cheat (AC) is mandatory for all matches
- Matches will not start if any player does not have AC enabled
- Players must run FACEIT AC during all matches — failure results in **match forfeit**
- Players with **active VAC bans or platform bans are ineligible**

### Camera Requirement (VRS Integrity Rule — NEW for Season 5)

- Players must have a working camera setup available upon request
- Admins may request players to turn on cameras at any time during matches for integrity verification, especially in VRS-eligible matches

### Forfeit Rule (Updated for Season 5)

- Any match not played within the scheduled round window without approved rescheduling will be recorded as a **forfeit loss**

### Head-to-Head Tiebreaker (NEW for Season 5)

- In the event of a tie in group standings, the head-to-head result between tied teams will be used as the **primary tiebreaker** before round differential

### Demo Recording

- All official matches are recorded and archived
- Demos available for admin review and dispute resolution
- Demo access provided through FACEIT platform

### Tournament Administration

- **Primary Rulebook:** Posted as a PDF in the Discord under `⁠CROSSFIRE⁠📖┃season-5-info`
- Tournament administration retains final authority on disputes
- Admin decisions are **final and confidential**
- CrossfireGG reserves the right to adjudicate on any issue not explicitly addressed by the rules
- All protests must be filed through official Discord ticket system within 1 day of match conclusion

### Broadcast & Streaming

- Official Broadcast & Streaming Rules Apply
- See `⁠CROSSFIRE⁠📺┃streaming` in Discord for full broadcast policy

---

## Season 5 vs Season 4 — Key Changes

### Prize Pool & Entry

| Item | Season 4 | Season 5 |
|------|----------|----------|
| Entry Fee | $100 | **$150** |
| Prize Pool | $3,500 | **$5,000** |

### Schedule & Structure

- **Season Length:** Season 4 was ~8 weeks; Season 5 is ~6 weeks (more condensed)
- **New:** Buffer / Make-up window added (June 13–18)
- **New:** Shorter break (4 days instead of a full week feel)

### Match Scheduling

- **Season 4:** Fixed Friday 8 PM EST default
- **Season 5:** Day 3 of each round window @ 8:00 PM EST

### Seeding / VRS

- **Season 4:** Used a fixed VRS date (Feb 2)
- **Season 5:** Using May 2nd VRS date

### Competitive Rule Updates

- **Forfeit Rule (Updated):** Any match not played within the scheduled round window without approved rescheduling will be recorded as a forfeit loss
- **Head-to-Head Tiebreaker (New):** Head-to-head result between tied teams is the primary tiebreaker before round differential
- **Camera Requirement (New, VRS Integrity Rule):** Working camera setup required upon admin request
- **Coach Substitution Rule (New):** Registered coaches may substitute as players if listed on the official roster pre-match and otherwise eligible

---

## Documentation

- Tournament Rules and Regulations — Posted as a PDF in Discord under `⁠CROSSFIRE⁠📖┃season-5-info`
- [Teams and Rankings](TEAMS.md)

---

© Crossfire Network LLC 2026 All Rights Reserved
